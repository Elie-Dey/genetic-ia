...
public class A_star {
	...
	public int heuristic(State a, City ville_initial) {
		...
		ArrayList<Links> links_results_prim = new ArrayList<Links>(); // we gonna put the links that prim will chose at
																		// each step
		...
		while (ville_visite_by_prim.size() < ville_non_visite.size()) { // Prim Algo
			...
			// System.out.println("min="+min_links.c1.name+" ["+min_links.distance+"]
			// "+min_links.c2.name);
