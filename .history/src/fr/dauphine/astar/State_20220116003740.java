...
public class State {
	...
	public State father; //state father so we can comeback at the end of the exploration and find the path
	public static HashMap<State, Integer> exploration = new HashMap<>(); // HashMap with state as key and f the value for A* as value
	...
	public State(City c,State father) { //first state will have ID=1
		...
		this.number_of_states++;
		this.id_state=number_of_states;
		this.visited_cities= new ArrayList<City>(father.visited_cities); //same father's Arraylist<City> , just we need to add this city now
		this.visited_cities.add(c);
		this.distance_from_root=father.distance_from_root+path_cost_with_city(c,father.my_city); // distance equal to distance from the intial state to the faher + the cost of the arc
	}
	...
	public void print_state() {
		System.out.println("I'm the state number: "+this.id_state+" and my city's : "+this.my_city.name+"[ID ="+this.my_city.ID+"] My distance from root ="+this.distance_from_root);
	}
	...
	public static void print_all_Map_exploration() {
		...
		for(Entry<State, Integer> entry: exploration.entrySet())System.out.println("f= "+entry.getValue()+" with :"+" I'm the state number: "+entry.getKey().id_state+" and my city's : "+entry.getKey().my_city.name+"[ID ="+entry.getKey().my_city.ID+"]");
	}
	...
	public static State minimum_exploration() { // null == error ; find the minimum value of f in the map and return the key wich's the state of this value 
		int min = Collections.min(exploration.values());
		for(Entry<State, Integer> entry: exploration.entrySet())
			if(min==entry.getValue())return entry.getKey();
		return null;
	}
}
